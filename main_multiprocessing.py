...
class scrapeProcess(multiprocessing.Process):
	def __init__(self, queue, asin, invact, url, profilesite, profilepage, profilexmltemplate):
		super(scrapeProcess, self).__init__()

		self.name = "ScrapeKwik-ASIN-"+str(asin)
		self.queue = queue	
		self.asin   = asin
		self.invact = invact
		self.url    = url
	...
	for row in dbrows:
		...
		if isactive == False: # True:
			...
			if checktime(rowtime, thistime):
				...
				proc=scrapeProcess(que, asin, invact, url, profilesite, profilepage, profiletemplate)
				processes.append((proc, que))
				proc.start()
